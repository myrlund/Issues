...
var openContract = function(url, closed) {
	var sure = true;
	if (closed)
		sure = confirm("Denne kontrakten er avsluttet. Er du sikker på at du vil fortsette?");
	
	if (sure)
		window.location = url;
}

var deleteContract = function(id, url) {
	sure = confirm("Er du sikker på at du vil slette denne kontrakten?\n\nDette kan ikke gjøres om.");
	if (sure) {
		$('#deleteContract').submit();
	}
}

var closeContract = function(id, url) {
	$('#closeContract').submit();
}
